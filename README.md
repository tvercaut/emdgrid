...
- **Bipartite Reference Solver**: Textbook transportation formulation (`emd_lemon`) over the occupied bins, solved with LEMON NetworkSimplex or CostScaling. Arcs are implicit and the $n \times m$ ground costs are computed on demand rather than stored, following POT's `EMD_wrap_lazy`. It ignores the grid's adjacency structure by design, which makes it the cross-check the structure-exploiting solvers are validated against, and it accepts arbitrary separable ground metrics. Note that the laziness is only on emdgrid's side of the call: LEMON copies the problem into its own arc-indexed arrays (~53 bytes per arc), so a $20^3$ squared-Euclidean instance still peaks at 3.4 GB. When memory-bound, use `mcf_dpartion` (identical separable problem on a layered DAG, 0.078 GB) or `emd_potlemon` below. See the header of `emd_lemon.hpp` for the full accounting.
- **Fully Lazy Bipartite Solver**: The same bipartite formulation (`emd_potlemon`) on the vendored POT network simplex, mirroring POT's `EMD_wrap_lazy`. Implicit arcs plus lazy cost, sparse flow, packed state and derived endpoint storage mean *nothing* is stored per arc, so peak memory is proportional to $n + m$ rather than $n \times m$: the $20^3$ squared-Euclidean instance runs in 0.041 GB against `emd_lemon`'s 3.4 GB, for a bit-identical cost at roughly 6x the runtime. Supports $L_1$ and squared Euclidean.
