...
TEST_CASE("mcf_l1 1D: identical histograms give zero") {
  const emdgrid::GridLayout<1> layout({5});
  const std::vector<double> v = {0.1, 0.2, 0.4, 0.2, 0.1};
  const emdgrid::GridDataView<1, double> h(layout, std::span(v));
  CHECK(emdgrid::mcf_l1(h, h) == doctest::Approx(0.0));
}

TEST_CASE("mcf_l1 2D: unit shift along one axis costs 1") {
  const emdgrid::GridLayout<2> layout({2, 2});
  const std::vector<double> h1v = {1.0, 0.0, 0.0, 0.0};
  const std::vector<double> h2v = {0.0, 1.0, 0.0, 0.0};
  const emdgrid::GridDataView<2, double> h1(layout, std::span(h1v));
  const emdgrid::GridDataView<2, double> h2(layout, std::span(h2v));
  CHECK(emdgrid::mcf_l1(h1, h2) == doctest::Approx(1.0));
}

TEST_CASE("mcf_l1 3D: diagonal shift costs 3") {
  ...
  CHECK(emdgrid::mcf_l1(h1, h2) == doctest::Approx(3.0));
}

TEST_CASE("mcf_l1 3D: matches emd_l1 on random histograms") {
  ...
  const double dist_mcf = emdgrid::mcf_l1(h1, h2);

  CHECK(dist_mcf == doctest::Approx(dist_emd).epsilon(1e-4));
}

TEST_CASE("mcf_l1 2D: transport plan computation and cost reconstruction") {
  const emdgrid::GridLayout<2> layout({2, 2});
  const std::vector<double> h1v = {1.0, 0.0, 0.0, 1.0};
  const std::vector<double> h2v = {0.0, 1.0, 1.0, 0.0};
  // Renormalize so h1v and h2v sum to 1.0
...
TEST_CASE("mcf_l1 2D: shape mismatch throws") {
  ...
  CHECK_THROWS_AS(static_cast<void>(emdgrid::mcf_l1(h4, h9)),
                  std::invalid_argument);
}

TEST_CASE("mcf_l1 2D: unnormalized mass throws") {
  const emdgrid::GridLayout<2> layout2({2, 2});
  ...
  CHECK_THROWS_AS(static_cast<void>(emdgrid::mcf_l1(h1, h2)),
                  std::invalid_argument);
}
...
TEST_CASE("mcf_dpartition L1: matches mcf_lemon_l1 and emd_l1 exactly") {
  ...
  CHECK(d_dpart_cs == doctest::Approx(2.0));
...
TEST_CASE("mcf_dpartition SqEuclidean: exact distance checks") {
  ...
  // 2D shift (0,0) -> (2,2): cost = (2-0)^2 + (2-0)^2 = 8
  const emdgrid::GridLayout<2> l2({3, 3});
  std::vector<double> h1_2d(9, 0.0);
  std::vector<double> h2_2d(9, 0.0);
  h1_2d[0] = 1.0;
  h2_2d[8] = 1.0;
  ...
  // 3D shift (0,0,0) -> (1,2,3): cost = 1^2 + 2^2 + 3^2 = 14
  const emdgrid::GridLayout<3> l3({2, 3, 4});
  std::vector<double> h1_3d(24, 0.0);
  std::vector<double> h2_3d(24, 0.0);
  h1_3d[l3.node({0, 0, 0})] = 1.0;
