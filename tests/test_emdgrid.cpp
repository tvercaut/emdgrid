...
TEST_CASE("library version matches generated version") {
  CHECK(emdgrid::version() == emdgrid::detail::version);
}
...
TEST_CASE("2D grid layout operations") {
  ...
  CHECK(layout2d.shape() == emdgrid::GridLayout<2>::Shape{2, 3});
  ...
  CHECK(layout2d.neighbours(layout2d.node({1, 1})) ==
...
TEST_CASE("3D grid layout operations") {
  ...
  CHECK(layout3d.node_count() == t3d_node_count);
  CHECK(layout3d.edge_count() == t3d_edge_count);
  CHECK(layout3d.node({1, 2, 3}) == t3d_last_node);
  CHECK(layout3d.coordinates(t3d_node) ==
